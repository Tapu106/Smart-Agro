...
app.post('/CreateCar', async function (req, res) {
    try {
        ...
        await contract.submitTransaction('createCrops', req.body.owner, req.body.ownername, req.body.cropKind, req.body.Quantity, req.body.price);
        console.log('Transaction has been submitted');
        // Disconnect from the gateway.
        await gateway.disconnect()
        res.redirect('/create');
...
app.get('/SellHistory', async function (req, res) {
    try {
        ...
        const result1 = await contract.submitTransaction('querySellHistory',user1.Id);
        console.log('Transaction has been submitted');
        // Disconnect from the gateway.
        await gateway.disconnect()
        return res.status(200).json({ response: JSON.parse(result1.toString()) });
...
app.get('/BuyHistory', async function (req, res) {
    try {
        ...
        const result1 = await contract.submitTransaction('queryBuyHistory', user.Id);
        await gateway.disconnect()
        console.log('Transaction has been submitted');
        return res.status(200).json({ response: JSON.parse(result1.toString()) });
...
app.post('/queryiD', async function (req, res) {
    try {
        ...
        const gateway = new Gateway();
        await gateway.connect(ccpPath, { wallet, identity: 'user1', discovery: { enabled: false, asLocalhost: true } });
...
app.post('/changeOwn', async function (req, res) {
    try {
        ...
        await contract.submitTransaction('BuyCrops', req.body.cropid,req.body.Buyer, req.body.quan);
        console.log('Query has been sucessfull');
